...
Using a central registry allowed us to limit the agents we could call to those from the class. This was useful as a classroom exercise, and could be useful in practice if there are only a specified number of agents that you want to be able to call for a particular task (e.g., they are very specialized or you trust them more than just any other agent). Decentralized discovery might also be useful, however, if you want to search through as many agents as possible to find the best agent for your task.
